...
give back a `meta` (for metadata, in a list) and `data` (for data, in a
data.frame) slot. The metadata:
...
Inspect data. A `tibble` is returned, so you get a nice brief data
summary:
...
Search for class *Aves*, in the state of *Nevada*, with a coordinate
uncertainty range (in meters) of less than 25 meters
...
Spatial search service allows only to search on a point defined by
...
[`vertsearch()`](https://docs.ropensci.org/rvertnet/reference/vertsearch.md)
provides a simple full text search against all fields. For more info see
[the
docs](https://github.com/VertNet/webapp/wiki/The-API-search-function#global-full-text-search).
An example:
...
and
[`vertsearch()`](https://docs.ropensci.org/rvertnet/reference/vertsearch.md),
you can request more than 1000 records. VertNet limits each request to
1000 records, but internally in this package, if you request more than
1000 records, we’ll continue to send requests to get all the records you
want. See the [VertNet
docs](https://github.com/VertNet/webapp/wiki/The-API-search-function#retrieving-large-result-sets)
...
In the previous examples, we’ve suppressed messages for more concise
output, but you can set `messages=TRUE` to get helpful messages -
`messages=TRUE` is also the default setting so if you don’t specify that
parameter messages will be printed to the console.
